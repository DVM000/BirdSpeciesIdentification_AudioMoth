...
# Resample audio
fs = 32000  # new sampling rate
song = resample_poly(song, fs, FS)
...
# Spectrogram
f, tspec, spec = scipy.signal.spectrogram(song, fs, nperseg=lwindow, noverlap=lwindow // 2)

# MFCC calculation
nbanks = 41
twindow = lwindow / fs

# Calculate MFCC and deltas using our functions
mfccs, delta = calculate_mfccs(song, nbanks, twindow, fs)
mfccs12 = mfccs[1:13,:]
deltas12 = delta[1:13,:]

y = NeuralNetworkFunction( np.concatenate([mfccs12, deltas12]) )  
y = y[0, :]
...
# Visualize results
nframes = int(spec.shape[1]/2) #60
t1 = np.linspace(0, len(y) * twindow, len(y))

plt.figure(figsize=(10, 6))
for i in range(len(t1) // nframes):
    plt.subplot(3, 1, 3)
    ...
    plt.subplot(3, 1, 1)
    plt.plot(t1[:nframes], detect[:nframes], 'g-', linewidth=1.5, label=f'NN output > {th}')
